...
const Experience = () => {
  return (
    <section id="experience" className="min-h-screen bg-dark text-light py-16">
      <div className="max-w-6xl mx-auto px-4">
        <h2 className="text-4xl font-bold text-center mb-12 text-brand">
          Experience
        </h2>
        <div className="space-y-8">
          {experiences.map((exp, idx) => (
            <div key={idx} className="p-6 bg-gray-800 rounded-2xl shadow">
              <h3 className="text-2xl font-semibold">
                {exp.role} — <span className="text-brand">{exp.company}</span>
              </h3>
              <p className="text-sm text-light/70 mb-3">{exp.date}</p>
